...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!roleData) {
      return new Response(JSON.stringify({ error: 'Acesso negado. Apenas super_admin pode cadastrar totens.' }), {
    ...
    const { nome, email, senha, cor } = body;
    ...
    // 4. Upsert profile com nome correto
    await adminClient
      .from('profiles')
      .upsert({ id: newUserId, nome_completo: nome.trim(), email: email.trim().toLowerCase() });

    // 5. Remover role 'paroquiano' do trigger e adicionar 'totem'
    ...
    if (roleError && !roleError.message.includes('duplicate') && !roleError.message.includes('unique')) {
      console.error('Erro ao inserir role totem:', roleError);
    }

    // 6. Inserir na tabela totens
    const { error: totemError } = await adminClient.from('totens').insert({
      user_id: newUserId,
      nome: nome.trim(),
      cor: cor || '#7B1C2A',
      ativo: true,
    ...
    if (totemError) {
      ...
      return new Response(JSON.stringify({ error: 'Erro ao cadastrar totem: ' + totemError.message }), {
